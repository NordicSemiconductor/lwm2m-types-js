/**
 * oA Physical Colour Light-Point Actuator
 *
 * The 'oA_Physical_Colour_Light-Point_Actuator' represents a physical colour
 * light-point actuator. It is necessary to map at least on Logical Light Point
 * Actuator to control the intensity and one Logical Colour Light-Point to
 * control the colour to the Physical Colour Light-Point.
 *
 * @see https://github.com/OpenMobileAlliance/lwm2m-registry/raw/prod/3396.xml
 *
 * ID: 3396
 * LWM2MVersion: 1.1
 * ObjectVersion: 1.0
 ...
 * Mandatory: false
 */
export type oAPhysicalColourLight_PointActuator_3396 = Readonly<
	Array<{
 ...
 * Current Colour Y
 *
 * This Resource represents the x value of the current colour. This is a
 * 'real-time' value, which is given independently of any ongoing change
 * request. Scaling is 0.0001 per unit
 *
 * ID: 122
 * MultipleInstances: false
 * Mandatory: true
 * Units: K
 */
type CurrentColourY_122 = number
/**
 * Current Colour X
 *
 * This Resource represents the y value of the current colour. This is a
 * 'real-time' value, which is given independently of any ongoing change
 * request. Scaling is 0.0001 per unit
 *
 * ID: 123
 ...
 * Units: K
 */
type CurrentColourX_123 = number
/**
 * Remaining Colour Transition Time
 *
 * Remaining approximated time of any ongoing colour change process which
 * requires a variation of the light output over a period of time higher or
 * equal to one 100ms interval. When the process is started, the value is equal
 * to the time value selected for the process and it is continuously decreased
 * until completed (time=0), or a 'Stop' request is
 * received.\n\t\t\t\t\n**NOTE:** If during a request the process reaches the
 * colour limits, the time is automatically set to '0' as the process is
 * stopped. Scaling is 100ms per unit
 *
 * ID: 124
 * MultipleInstances: false
 * Mandatory: true
 ...
 */
export const objectURN = '3396@1.1'
